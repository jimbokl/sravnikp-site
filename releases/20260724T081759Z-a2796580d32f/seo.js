...
const PAID_AUDIT_EMAIL = "sravnikp@upuugaree.resend.app";
...
function initializeGenericCalculator(calculator) {
  ...
  function calculate() {
    ...
    const low = base * 0.88 + extraCost;
    const high = base * 1.18 + extraCost;
...
function renderAudit(tool, result, warnings, fileName) {
  ...
  const paidAudit = tool.querySelector("[data-paid-audit-cta]");
  if (paidAudit) {
    const subject = encodeURIComponent(`Ручной разбор сметы за 1000 ₽ — ${fileName}`);
    const body = encodeURIComponent([
      "Здравствуйте! Хочу заказать пилотный ручной разбор сметы за 1000 ₽.",
      "",
      `Файл: ${fileName}`,
      `Распознано строк: ${result.rows.length}`,
      `Пересчитанный итог: ${currency.format(result.total)}`,
      "Населённый пункт в Москве или Московской области:",
      "Что вызывает сомнение:",
      "",
      "Прикрепляю исходный файл вручную. Понимаю, что после отправки по email он покинет браузер и будет обработан человеком.",
    ].join("\n"));
    paidAudit.href = `mailto:${PAID_AUDIT_EMAIL}?subject=${subject}&body=${body}`;
  }
